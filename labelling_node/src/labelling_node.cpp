...
// OpenPose
DEFINE_string(model_folder,             "/home/ait/cam/flycams/openpose/models/",      "Folder path (absolute or relative) where the models (pose, face, ...) are located.");
...
{
    double camera_quaternion[4] = {0.0, 0.0, 0.0, 0.0};
...
class WUserInput : public op::WorkerProducer<std::shared_ptr<std::vector<UserDatum>>>
{
    ...
    ros::Subscriber mCameraQuaternionSub;
...
public:
    ...
    WUserInput()
        : cv_ptr(new cv_bridge::CvImage()), r(30)
    {
        ros::NodeHandle nh_;
        image_transport::ImageTransport it_(nh_);
        int id;
        nh_.getParam("/labelling_node_node/bebop_id", id);
        std::string ns = nh_.getNamespace();
        mCameraQuaternionSub = nh_.subscribe(ns + "/real/metadata/camera_quat", 1, &WUserInput::quaternionCallback, this);
//        mCameraQuaternionSub = nh_.subscribe("/q" + std::to_string(id) + "/real/metadata/camera_quat", 1, &WUserInput::quaternionCallback, this);
        mImageSub = it_.subscribe(ns + "/real/image_raw", 1, &WUserInput::imageCallback, this);
//        mImageSub = it_.subscribe("/q" + std::to_string(id) + "/real/image_raw", 1, &WUserInput::imageCallback, this);
        op::log(mImageSub.getTopic());
        sleep(3);
    }
...
//            r.sleep();
            ...
            cv::Mat croppedImg = img(cv::Rect(213, 0, 428, 480));
            datum.cvInputData = croppedImg;
            datum.camera_quaternion[0] = mQuaternion.quaternion.x;
            datum.camera_quaternion[1] = mQuaternion.quaternion.y;
            datum.camera_quaternion[2] = mQuaternion.quaternion.z;
            datum.camera_quaternion[3] = mQuaternion.quaternion.w;

            // If empty frame -> return nullptr
            if (datum.cvInputData.empty())
            {
                op::log("No image received from subscriber");
                this->stop();
                datumsPtr = nullptr;
            }
...
{
    ros::Publisher mKeypointPublisher;
    std_msgs::Float64MultiArray msg;
    int label_lookup[13] = {10, 9, 8, 4, 3, 2, 0, 5, 6, 7, 11, 12, 13};
...
public:
    ...
    WUserOutput()
    {
        ...
        std::string ns = nh_.getNamespace();
        nh_.getParam("/labelling_node_node/bebop_id", id);
//        mKeypointPublisher = nh_.advertise<std_msgs::Float64MultiArray>("/q" + std::to_string(id) + "/labels", 1);
        mKeypointPublisher = nh_.advertise<std_msgs::Float64MultiArray>(ns + "/labels", 1);
    ...
    void workConsumer(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr)
    {
        try
        {
            ...
            if (datumsPtr != nullptr && !datumsPtr->empty())
            {
                ...
                const auto numberXYScore = poseKeypoints.getSize(2);



                // Take Person with highest score
                int bestPerson = 0;
                float scoreMax = 0;
                std::vector<double> data(2*13,0.0);

                if (numberPeopleDetected==1) {
                    for (int labelNr = 0; labelNr < 13; labelNr++) {
                        int part = label_lookup[labelNr];
                        const auto baseIndex = numberXYScore*part;
                        const auto x = poseKeypoints[baseIndex];
                        const auto y = poseKeypoints[baseIndex + 1];
                        data[labelNr*2] = (x+214); // need to mirror image
                        data[labelNr*2+1] = y+1;
                    }
                }
                // Append Quaternion
                double* q = datumsPtr->at(0).camera_quaternion;
                data.push_back(q[0]);
                data.push_back(q[1]);
                data.push_back(q[2]);
                data.push_back(q[3]);
                ...
                // Publish the labels
                msg.data = data;
                mKeypointPublisher.publish(msg);
